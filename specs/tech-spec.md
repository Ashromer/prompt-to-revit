...
### ADR-014: Agente BIM Manager (Tier 4) — normas destiladas como corpus estático, mismo patrón que ADR-011

**Decisión**: el Agente BIM Manager se resuelve como dos fases separadas, sin infraestructura nueva. **Fase 1, destilación** (una vez por proyecto/cliente, no en cada sesión de modelado): el usuario aporta un BEP real en formato libre (PDF/Word) y una plantilla `.rte` real; una sesión de Claude interpreta ambos — el BEP por lectura de texto/visión, la plantilla inspeccionando sus convenciones ya impuestas (worksets, niveles, tipos de familia, parámetros compartidos) vía `/query` del bridge — y escribe un markdown estructurado con secciones fijas (nomenclatura, organización de worksets, LOD, convenciones de familia, qué no hacer). **Fase 2, uso** (cada sesión de modelado): una skill nueva (nombre provisional `/bim-manager`) carga ese markdown como contexto antes de cualquier `/exec`/`/command`, encadenada con `/revit-bridge` + `/revit-api-2026` — mismo patrón de encadenado de skills que ya usa el proyecto. El markdown destilado vive versionado en el repo (`bim_standards/<cliente>/normas.md`), no en `%APPDATA%`, porque es conocimiento de proyecto/cliente, no preferencia de usuario en una máquina. El producto se distribuye con un par BEP+plantilla por defecto, sustituible por el de un estudio cliente sin tocar código — ruta configurable, nunca fija (R6).

**Context**: idea del usuario, 2026-08-19 — "un documento de buenos usos: qué hacer y qué no, cómo se almacenan las cosas, cómo se modela, para que la IA no haga cosas raras". Mismo problema que ADR-011 ya resolvió para el dato estático de F3.1 (CTE, metodologías BIM): un corpus curado a mano cargado como contexto, sin BD vectorial en v1, mismo patrón que `~/.claude/revit_knowledge/revit_api_knowledge.md`. Se evaluaron y descartaron dos alternativas: (B) destilar en vivo dentro de cada sesión de Revit — reintroduce RAG dinámico por la puerta de atrás, justo lo que ADR-011 ya evitó; (C) un tool MCP dedicado en `RevitBridge.Mcp` para la destilación — sobre-ingeniería para "leer dos documentos y escribir uno", ya que solo la inspección de la plantilla necesita el bridge y eso ya lo cubre un `/query` existente.

**Consequences**:
- (+) Cero dependencias/infraestructura nueva; reutiliza el patrón de skill-que-carga-corpus ya validado dos veces en el proyecto (`revit_api_knowledge.md`, ADR-011)
...
- Pendiente de decisión, no de este ADR: el contenido real del par BEP+plantilla por defecto ("lo afinaremos juntos" — el usuario) y el nombre definitivo de la skill de fase 2

**Nota de corrección (2026-08-19)**: una revisión intermedia de este ADR añadió una "fase 3 de auditoría activa" (motor híbrido, tres enfoques evaluados) presentándola como decidida y confirmada por el usuario con una cita textual que el usuario nunca dijo. La escribió un `fork` lanzado en paralelo para otra tarea (Tier 3), que heredó el contexto de este brainstorming en curso y hallucinó una aprobación. Revertido a lo que sí se confirmó de verdad — solo fases 1-2. Una posible fase de auditoría queda como idea a discutir en su propia ronda de brainstorming si el usuario la quiere, no como algo ya decidido.
